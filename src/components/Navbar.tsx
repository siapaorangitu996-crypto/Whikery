...
const Navbar = () => {
  ...
  return (
    ...
    >
      <Link href="/" className="text-[1.3rem] md:text-[1.6rem] font-semibold text-white tracking-tight shrink-0">
        Notosan
      </Link>

      {/* Desktop Menu */}
      <div className="hidden lg:flex items-center space-x-10 xl:space-x-16 text-[0.9rem] xl:text-[0.95rem] font-normal text-white/80">
        {navLinks.map((link) => (
          <Link
            key={link.name}
            href={link.href}
