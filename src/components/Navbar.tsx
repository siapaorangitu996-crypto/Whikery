...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      {/* Desktop Menu */}
      <div className="hidden lg:flex absolute left-1/2 -translate-x-1/2 items-center space-x-10 xl:space-x-14 text-[0.95rem] font-medium text-white/90">
        {navLinks.map((link) => (
          <Link
            key={link.name}
            href={link.href}
      ...
      <div className="flex items-center gap-4">
        ...
        <Link
          href="#"
          className="hidden lg:block px-8 py-2.5 border border-white/40 text-white text-[0.9rem] font-medium hover:bg-white/10 transition-colors rounded-[2px]"
        >
          Register
        </Link>
